...
export function useMessages(coupleId: string | null, vaultId: string | null = null) {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    setLoading(true);
    let query = supabase.from("messages").select(`*, sender:profiles(id, display_name, avatar_url)`).eq("couple_id", coupleId).order("created_at", { ascending: true });
    if (vaultId) query = query.not("vault_id", "is", null).eq("vault_id", vaultId);
  ...
  const sendMessage = useCallback(async (content: string, attachmentId?: string, replyToId?: string, encryptedPayload?: { encryptedContent: string; encryptedKey: string; nonce: string }) => {
  ...
  const editMessage = useCallback(async (messageId: string, content: string, encryptedPayload?: { encryptedContent: string; encryptedKey: string; nonce: string }) => {
  ...
  const deleteMessage = useCallback(async (messageId: string, forBoth: boolean) => {
    if (forBoth) {
      const { error } = await supabase.from("messages").update({ deleted_for_both: true, is_deleted: true }).eq("id", messageId);
      return !error;
    } else {
